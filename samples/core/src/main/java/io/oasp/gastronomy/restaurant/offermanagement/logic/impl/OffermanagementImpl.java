...
public class OffermanagementImpl extends AbstractComponentFacade implements Offermanagement {
  ...
  /** @see #setSpecialDaoDao(SpecialDao) */
  ...
  public SpecialEto findSpecial(Long id) {
    ...
    SpecialEntity special = this.specialDao.findOne(id);
    if (special == null) {
      return null;
    }
  ...
  public List<SpecialEto> findAllSpecials() {
    ...
    List<SpecialEntity> specials = this.specialDao.findAll();
    return getBeanMapper().mapList(specials, SpecialEto.class);
  }

  @Override
  public void deleteSpecial(Long specialId) {

    SpecialEntity special = this.specialDao.find(specialId);
    if (special != null) {
      this.specialDao.delete(special.getId());
    }
  }
  ...
  @Override
  public List<SpecialEto> findSpecialsEtos(SpecialSearchCriteriaTo criteria) {

    List<SpecialEntity> results = this.specialDao.findByCriteria(criteria);
    return getBeanMapper().mapList(results, SpecialEto.class);
  }
